...
export async function loadConfig() {
  ...
  return {
    targetUsername,
    enableDashboard: readBoolean("ENABLE_DASHBOARD", true),
    dashboardPort: readPositiveNumber("DASHBOARD_PORT", 3000),
    openBrowserOnExit: readBoolean("OPEN_BROWSER_ON_EXIT", true),
    enableGoogleSheets: readBoolean("ENABLE_GOOGLE_SHEETS", false),
    ...
    waitForLive: readBoolean("WAIT_FOR_LIVE", true),
    livePollIntervalMs: secondsToMs(readPositiveNumber("LIVE_POLL_INTERVAL_SECONDS", 30)),
    retainLiveSessions: Math.max(1, Math.floor(readPositiveNumber("RETAIN_LIVE_SESSIONS", 5))),
    ...
    enableFullTranscriptLog: readBoolean("ENABLE_FULL_TRANSCRIPT_LOG", true),
    enableJsonExport: readBoolean("ENABLE_JSON_EXPORT", true),
    enableCsvExport: readBoolean("ENABLE_CSV_EXPORT", true),
    enableSheetExport: readBoolean("ENABLE_SHEET_EXPORT", true),
    sheetTopLimit: readPositiveNumber("SHEET_TOP_LIMIT", 30),
